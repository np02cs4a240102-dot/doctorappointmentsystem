...
$pm_end          = $input['pm_end']                    ?? '';
$password        = $input['password']                  ?? 'doctor123';
$confirm         = $input['confirm_password']          ?? $password;

// ── Validate ───────────────────────────────────────────
if (!$name || !$email || !$phone || !$specialization || !$am_start || !$am_end) {
...
if ($experience_years < 0 || $experience_years > 60) {
    errorResponse('Experience years must be between 0 and 60.', 400, 'experience_years');
}
...
$am_s     = new DateTime($am_start);
$am_e     = new DateTime($am_end);
foreach (new DatePeriod($am_s, $interval, $am_e) as $dt) {
    $slots[] = $dt->format('g:i A');
}

if ($pm_start && $pm_end) {
    $pm_s = new DateTime($pm_start);
    $pm_e = new DateTime($pm_end);
    foreach (new DatePeriod($pm_s, $interval, $pm_e) as $dt) {
        $slots[] = $dt->format('g:i A');
    }
}
...
// ── Insert doctor ──────────────────────────────────────
$stmt3 = $db->prepare('INSERT INTO doctors (name, email, phone, specialization, experience_years, allowed_days, allowed_slots, password, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, "ACTIVE")');
$stmt3->bind_param('ssssisss', $name, $email, $phone, $specialization, $experience_years, $days_str, $slots_str, $hashed);
